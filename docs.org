...
**** Branding
     *http.async.client* con be configured, for now only with
     User-Agent. To do so you use [[http://neotyk.github.com/http.async.client/autodoc/http.async.client-api.html#http.async.client/create-client][*http.async.client/create-client*]].
     It takes map of options, for now it only accepts *:user-agent*.
     Once you've created client you need to use it via
     [[http://neotyk.github.com/http.async.client/autodoc/http.async.client-api.html#http.async.client/with-ahc][*http.async.client/with-ahc*]].
#+BEGIN_SRC clojure
